...
# library(expm)
# library(Matrix)
>>>>>>> 295773213810ffc4c34e5ac0006184c14d980fd1
library(stats)
library(boot)
library(latex2exp)
library(coda)
...
residual_bootstrap_resampling = function(x, p, B){
  ...
  for(i in 1:B){
    ...
    means_LS = c(means_LS,mean(x_LS))
    samples_LS = c(samples_LS, x_LS)
    x_LA = ARp.filter(x0 = x_pair, 
                         beta = beta_vecs$LA, 
                         e = sample(eps_LA, size=length(eps_LA), replace=TRUE))
    means_LA = c(means_LA,mean(x_LA))
    samples_LA = c(samples_LA, x_LA)
  }
  df_boot = data.frame(LS = samples_LS, LA = samples_LA, est_LS = mean(means_LS)/B,
                       est_LA = mean(means_LA)/B)
  return(df_boot)
}
...
#computing bias:
bias_LS = boot1$est_LS - data3A$x
bias_LA = boot1$est_LA - data3A$x
print(bias_LS)
print(bias_LA)
