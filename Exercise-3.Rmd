...
permutation.test = function(df, Fval, B = 999, plot.hist = TRUE) {
  ...
  if (plot.hist) {
    fig.hist = ggplot(data.frame(fstatistic = fstatistic.vec), aes(x = fstatistic, y = ..density..)) +
      ...
      geom_vline(aes(xintercept=Fval, col = "F-statistic calculated from original dataset"), data = data.frame(Fval = Fval[1]), linetype = "dashed") + theme(legend.position = "bottom") +
      
      guides(col = guide_legend("Vertical line"))
