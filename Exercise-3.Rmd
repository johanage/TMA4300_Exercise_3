...
\begin{equation}
\mathrm{bias}(\hat{\beta}_{LS}) =  \hat{\beta}_{LS} - \hat{\beta}_{LS}^{*}(\cdot),
\end{equation}
where $\hat{\beta}_{LS}^{*}(\cdot)$ represents the bootstrap mean of the beta estimator.

5. Calculate bias for LA:
\begin{equation}
\mathrm{bias}(\hat{\beta}_{LA}) =  \hat{\beta}_{LA} - \hat{\beta}_{LA}^{*}(\cdot),
\end{equation}
where $\hat{\beta}_{LA}^{*}(\cdot)$ represents the bootstrap mean of the beta estimator.
...
1. Generate residual distribution for LS and LA by sampling from the computed residuals from time series data.
2. Sample from the residual distribution to simulate a value $x_{101}$ by using the AR(2) scheme.
...
4. Find the limits in the prediction interval for $x_{101}$ as quantiles in the simulated $x_{101}$ values.
...
calc_x101 = function(res_LS, res_LA, x_LA, x_LS, beta_LS, beta_LA, B, Timesteps){
  x101_ls = c()
  x101_la = c()
  for(i in 1:B){
    e_101_LS = sample(res_LS, size = 1, replace = TRUE)
    e_101_LA = sample(res_LA, size = 1, replace = TRUE)
    xls = x_LS[i:Timesteps*i]
    xla = x_LS[i:Timesteps*i]
    bls = beta_LS[i:(i+1)]
    bla = beta_LA[i:(i+1)]
    x101_ls = c(x101_ls, bls[2]*xls[Timesteps] + bls[1]*xls[Timesteps-1] + e_101_LS)
    x101_la = c(x101_la, bls[2]*xla[Timesteps] + bla[1]*xla[Timesteps-1] + e_101_LA)
  }
  return(list(x101_LS = x101_ls,
              x101_LA = x101_ls))
}

x101s = calc_x101(res_LS = boot1$resiudals_LS, 
                     res_LA = boot1$resiudals_LA, 
                     x_LA = boot1$LA, 
                     x_LS = boot1$LS,
                     beta_LS = boot1$betas_LS_est, 
                     beta_LA = boot1$betas_LA_est, 
                     B = 2000, Timesteps = 100)

```


```{r plot_x101_LS, fig.cap="Plot of the $x_{101}$'s simulated by LS."}
df_x101 = data.frame(ls = x101s$x101_LS, la = x101s$x101_LA )
...
ggplot(df.bilirubin, aes(x = pers, y = log(meas))) + geom_boxplot() + ggtitle("Boxplot of logarithm of concentration") + ylab("log(measurement)") + xlab("Person")
```
