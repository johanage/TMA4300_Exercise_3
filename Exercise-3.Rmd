...
In order to do the resampling we have to pick random consecutive pairs from the original data. In this case where $\mathbf{x} = \{x_1, \dots, x_{100} \}$ we pick randomly from all possible pairs is given by:
...
print(x1x2_sample1)
```
To do a resampling, we initialize $x_3$ by using a pair of values $x_1$ and $x_2$ by picking a random consecutive subsequence from the data. Then we will iteratively compute the next $x_i$ following the AR(2) scheme. This is done by the function ARp.filter().
...
\begin{equation}
\hat{\mathrm{Bias}}(\hat{\beta}_{LS}) =  \hat{\beta}_{LS}^{*}(\cdot) - t(\hat{F}),
\end{equation}
where $\hat{\beta}_{LS}^{*}(\cdot)$ represents the bootstrap mean of the beta estimator of the LS method.

5. Calculate bias for LA:
\begin{equation}
\hat{\mathrm{Bias}}(\hat{\beta}_{LA}) =  \hat{\beta}_{LA}^{*}(\cdot) - t(\hat{F}),
\end{equation}
where $\hat{\beta}_{LA}^{*}(\cdot)$ represents the bootstrap mean of the beta estimator for the LA method and $t(\hat{F})$ is the calculated betas from the original data.
