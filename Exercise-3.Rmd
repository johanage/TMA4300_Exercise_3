...
```{r plot_OG, fig.height=3, fig.width=4, fig.cap=" \\label{fig:plot_OG} Plot of the non-Gaussian time series."}
=======
```{r plot_OG, fig.height=3, fig.width=4, fig.cap="\\label{fig:plot_OG} Plot of the non-Gaussian time series."}
>>>>>>> d7c802eae39e4ebdef7400ea4c3b8b6bb87a3560
source("probAdata.R")
...
In this problem we are going to analyze the non-Gaussian time-series data in `data3A\$x`. The data contains a subsequence of length $T = 100$. The target is to compare two different estimators. A plot of the data is given in Figure \ref{fig:plot_OG}.
...
where $e_t$ are iid with zero mean and constant variance. We are going to use the the least sum of squared residuals (LS) and least sum of absolute residuals (LA) to compute the two estimators of the parameters of the residuals we want to compare. The estimator we want to focus on is the minimisers of the LS and LA. The minimisers are obtained by minimising the following loss functions with respect to $\vect{\beta}$:
...
We denote the minimisers by $\vect{\beta}_{LS}$ and $\vect{\beta}_{LA}$ (calculated by the script \textbf{ARp.beta.est}). We then define the estimated residuals to be:
...
to have mean zero by defining $\hat{\epsilon}_t = \hat{e}_t -\bar{e}$. (Results for $\hat{\epsilon}_t$ obtained by LS and LA is calculated with \textbf{ARp.resid}).

## A.1

### 1.1 Use the residual resampling bootstrap method to evaluate the relative performance of the two parameter estimators.

In order to do the resampling we pick a random consecutive pair from the original data to initialize the resampling. In this case where $\mathbf{x} = \{x_1, \dots, x_{100} \}$ we pick randomly from all possible pairs is given by:
...
The resampling is then done by initializing $x_3$ by using a pair of values $x_1$ and $x_2$ which is given from the randomly picked consecutive subsequence from the original data. Then we will iteratively compute the next $x_t$ following the AR(2) scheme. This is done by the function \textbf{ARp.filter()}.
...
```{r, fig.height=3, fig.width=4, fig.cap=" \\label{LS_resampling} Plot of the samples generated from the LS resampling."}
...
```{r, fig.height=3, fig.width=4, fig.cap=" \\label{LA_resampling} Plot of the samples generated from the LA resampling."}
...
<<<<<<< HEAD
The two parameters of interest are $\vect{\beta}$ and the estimators for the minimisers is $\hat{\vect{\beta}}_{LS}$ and $\hat{\vect{\beta}}_{LA}$ with respect to the loss functions $Q_{LS}$ and $Q_{LA}$. We denote the minimiser function of a general loss function $Q(\vect{x})$ as $s(\vect{x})$ calculated by using the original data and $s(\vect{x}^{b*})$ calculated by using the bootstrapped samples $\vect{x}^{b*}$. We want to estimate the variance and the bias of the estimators. This is given by $\mathrm{Var}(\hat{\vect{\beta}}_{LS})$ and $\mathrm{Var}(\hat{\vect{\beta}}_{LA})$ where we use the plug-in principle for the estimation. The following algorithm for calculating the variance and bias for the estimators in the residual bootstrapping is then given by:

1. Generate $B$ bootstrap samples $\vect{x}^{1*}, \dots, \vect{x}^{B*}$ for both LS and LA method using the estimators $\hat{\vect{\beta}}_{LS}$ and $\hat{\vect{\beta}}_{LA}$.
2. Evaluate the corresponding parameter estimates both methods: $\hat{\beta}^{*}(b) = s(\vect{x}^{b*}) , b = 1,2, \dots, B$. 
3. Estimate the variance for both methods by:
=======
The two parameters of interest are $\vect{\beta} = [\beta_1, \beta_2]$ and the estimators for the minimisers is $\hat{\vect{\beta}}_{LS} = s_{LS}(\vect x)$ and $\hat{\mathbf{\beta}}_{LA} = s_{LA}(\vect x)$ with respect to the loss functions $Q_{LS}$ and $Q_{LA}$. We want to estimate the variance and the bias of the estimators. This is given by $\mathrm{Var}(\hat{\mathbf{\beta}}_{LS})$ and $\mathrm{Var}(\hat{\mathbf{\beta}}_{LA})$ where we use the plug-in principle for the estimation. The following algorithm for calculating the variance and bias for the estimators in the residual bootstrapping is then given by:

1. Generate $B$ bootstrap samples $\vect x^{1*}, \dots, \vect x^{B*}$.
2. For each estimator $\hat {\vect \beta} ^* \in \{ \hat {\vect \beta}_{LS}^*, \hat {\vect \beta}_{LA}^* \}$ and corresponding $s \in \{ s_{LS}, s_{LA}\}$, evaluate the corresponding parameter estimates: $\hat{\beta}^{*}(b) = s(x^{b*}), b = 1,2, \dots, B$.
...
\begin{equation}
\widehat{\mathrm{Bias}}(\hat{\vect{\beta}}) =  \hat{\vect{\beta}}^{*}(\cdot) - t(\hat{F}),
\end{equation}

where $\hat{\vect{\beta}}^{*}(\cdot)$ represents the bootstrap mean of the beta estimator of the LS and LA method  and $t(\hat{F})$ is the calculated estimators from the original data.
...
3. Find the limits in the prediction interval for $x_{101}$ as quantiles in the simulated $x_{101}$ values by using the \textbf{stats::quantile()}-function using $\textrm{probs} = \textrm{c}(0.025, 0.975)$.
...
df_x101 = data.frame(x101_ls = boot1$x101_LS, x101_la = boot1$x101_LA, q_LS = q_x101_LS, q_LA = q_x101_LA)

print(df_x101$q_LS[1:2])
print(paste0("The 95% prediction interval for LS is ", q_x101_LS[1], " to ", q_x101_LS[2]))

ggplot(data = df_x101) + geom_histogram(aes(x = x101_ls)) + 
...
permutation.test = function(df, Fval, B = 999, plot.hist = TRUE) {
  ...
  if (plot.hist) {
    fig.hist = ggplot(data.frame(fstatistic = fstatistic.vec), aes(x = fstatistic, y = ..density..)) +
      ...
      geom_vline(aes(xintercept=Fval, col = "F-statistic calculated from original dataset"),
...
Let $x_1, \dots, x_n$ and $y_1, \dots, y_n$ be independent random variables where the $x_i$'s have an exponential distribution with intensity $\lambda_0$ and the $y_i$'s have an exponential distribution with intensity $\lambda_1$. Assume we do not observe $x_1, \dots, x_n$ and $y_1, \dots, y_n$ directly, but that we observe
...
optim.result = optim(initial.lambdas, log.likelihood, NULL, z, u,
                     
                     control = list(fnscale = -1), hessian=TRUE, lower=c(0,0))
