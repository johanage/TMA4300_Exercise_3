...
As the bias estimates are quite small compared to the estimates of $\lambda_0$ and $\lambda_1$ we prefer the maximum likelihood estimates $(\hat \lambda_0, \hat \lambda_1) = (`r round(result.em$lambda0, 4)`, `r round(result.em$lambda1, 4)`)$ over the bias corrected estimates $(\lambda_0^c, \lambda_1^c) = (`r round(result.bootstrap$lambda0.bias.est.improved,4)`, `r round(result.bootstrap$lambda1.bias.est.improved,4)`)$ since $\Var[\hat \lambda_0^c] \geq \Var[\hat \lambda_0]$ and $\Var[\hat \lambda_1^c] \geq \Var[\hat \lambda_1]$.
