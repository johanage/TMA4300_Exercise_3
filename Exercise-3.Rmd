...
The rest of each bo is then done by initializing $x_3$ by using a pair of values $x_1$ and $x_2$ which is given from the randomly picked consecutive subsequence from the original data. Then we will iteratively compute the next $x_t$ following the AR(2) scheme. This is done by the function \textbf{ARp.filter()}.
