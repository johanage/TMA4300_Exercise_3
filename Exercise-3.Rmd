...
The minimisers $\vect{\beta}_{LS}$ and $\vect{\beta}_{LA}$ are calculated by the given function \textbf{ARp.beta.est}. We then define the estimated residuals to be:
...
The rest of each bootstrapped time series is then computed iteratively by the AR(2) scheme in Equation (\ref{AR2}) and resampling with replacement from the residuals computed from the original time series. This is done for both methods LA and LS. We do this using the given function \textbf{ARp.filter}.

To evaluate the performance of the two estimators $\hat {\vect{\beta}}_{LS}$ and $\hat {\vect{\beta}}_{LA}$ we choose to look at the estimates for the variance and the bias of each estimator. We will use our bootstrapped time series to do this. We denote the minimiser function of a general loss function $Q(\vect{x})$ as $s(\vect{x})$. The following algorithm for calculating the variance and bias for the estimators in the residual bootstrapping is then given by
...
4. Calculate bias for both methods by:
\begin{equation}
\widehat{\mathrm{Bias}}(\hat{\vect{\beta}}) =  \hat{\vect{\beta}}^{*}(\cdot) - t(\hat{F}),
\end{equation}

where $\hat{\vect{\beta}}^{*}(\cdot)$ represents the bootstrap mean of the beta estimator of the LS or LA method  and $t(\hat{F})$ is the calculated LS or LA estimator for the original time series.
...
```{r, fig.height=3, fig.width=4, fig.cap=" \\label{LS_resampling} Plot of a random bootstrapped time series using the LS model."}
...
```{r, fig.height=3, fig.width=4, fig.cap=" \\label{LA_resampling} Plot of a random bootstrapped time series using the LS model."}
...
The LS estimator is optimal for Gaussian AR(p) processes. From the calculated variance of the estimators it is not evident which method is better. The LS method has a slightly larger variance than the LA method. The bias is also slightly larger for the LS method than the LA method. However these differences can just be due to randomness in the bootstrap. Consequently, the LS method does not appear to be more optimal than the LA method.

## 2)

To be able to compute the confidence interval for $x_{101}$ we choose to use the results from Problem A1. To simulate the value of $x_{101}$ we need to estimate the residual distribution corresponding to the bootstrapped time series and parameter estimates. The strategy we will follow for this problem is:
