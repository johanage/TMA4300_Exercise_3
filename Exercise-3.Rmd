...
residual_bootstrap_resampling = function(x, p, B){
  ...
  Outputs:
  - LS = samples using the LS method, type = vector, size = length(x)*B 
  - LA = samples using the LA method , type = vector, size = length(x)*B 
  - betas_LS_est = beta estimators using the LS method, type = vector, size = 2*B
  - betas_LA_est = beta estimators using the LA method, type = vector, size = 2*B
  - est_var_beta_LS = estimated variance using the estimators from the LS method, type = double
  - est_var_beta_LA = estimated variance using the estimators from the LA method, type = double
  - bias_beta_LS = estimated bias using the estimators from the LS method, type = double
  - bias_beta_LA = estimated bias using the estimators from the LA method, type = double
  - resiudals_LS = esimated residuals using the LS method, type = vector, size = length(x)*B
  - resiudals_LA = esimated residuals using the LA method, type = vector, size = length(x)*B 
  - x101_LS = simulated value of x101 using the data from the LS method, type = double 
  - x101_LA = simulated value of x101 using the data from the LA method, type = double "
  ...
  return(list(LS = samples_LS, LA = samples_LA,
              ...
              est_var_beta_LS = hat_SE_B_LS,
              est_var_beta_LA = hat_SE_B_LA,
...
print(paste0("Variance of the betas: LS = ", boot1$est_var_beta_LS[1]," and LA = ", boot1$est_var_beta_LA[1]))
...
From the calculated variance of the estimators it is not evident which method is better. The LS method has a slightly larger variance than the LA method. The bias is also slightly larger for the LS method than the LA method. However these differences can justs be due to randomness in the bootstrap.

## A.2 
### Computation of a $95 \%$ confidence interval for $x_{101}$ based upon both estimators.
...
df_x101 = data.frame(x101_ls = boot1$x101_LS, 
                     x101_la = boot1$x101_LA, 
                     q_LS = q_x101_LS, 
                     q_LA = q_x101_LA)
...
The distribution of the $x_{101}$ samples for both teh LS and the LA method is shown in Figure \ref{plot_x101_LS} and \ref{plot_x101_LA}.
